...
mv StandardMibs mibs/
mkdir mibs/modules
set +x
echo "Copying mibs to mibs/modules/(MODULE NAME)"
for a in mibs/StandardMibs/* ; do n=$(awk '/DEFINITION.*BEGIN/ { print $1; exit 0; }' $a); cp $a mibs/modules/$n || echo "failed: $a" ; done
mv mibs ${ORIGPWD}/
cd ${ORIGPWD}
rm -rf ${TMP}
